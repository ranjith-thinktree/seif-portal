...
const essciFileFilter = (req, file, cb) => {
  ...
  if (file.fieldname === 'zipFile') {
    ...
    if (!allowed.includes(ext)) {
      return cb(
        new Error(`Student result sheet must be Excel or CSV (.xlsx, .xls, .csv). Got: ${ext}`),
        false
      );
    }
    ...
    if (!allowed.includes(ext)) {
      return cb(
        new Error(`Certificate files must be ZIP archive or PDF. Got: ${ext}`),
        false
      );
    }
  }
...
/** Two-field upload for ESSCI legacy: zipFile + studentListDoc + certificateFiles */
