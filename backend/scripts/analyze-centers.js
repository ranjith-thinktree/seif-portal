...
const csvPath = path.join(
  __dirname,
  '../../documents/Final Sheet 25-26 -last 3 years details .csv'
);
...
csvCenters.forEach((csv) => {
  ...
  if (dbCentersByName.has(csvNameLower)) {
    ...
    if (dbMatches.length === 1) {
      ...
      const cityMatch = dbMatches.find(
        (db) =>
...
results.exactMatches.forEach((match, i) => {
  console.log(
    `${i + 1}. CSV Row ${match.csv.csvIndex}: "${match.csv.centerName}" | City: ${match.csv.city}`
  );
...
results.noMatches.forEach((csv, i) => {
  ...
  if (similarNames.length > 0) {
    console.log(
      `   Similar in DB: ${similarNames.map((s) => `"${s.centerName}" (${s.city})`).join(', ')}`
    );
  }
  console.log();
});
...
dalmiaCsv.forEach((c) => {
  console.log(
    `  - Row ${c.csvIndex}: "${c.centerName}" | Location: ${c.location} | City: ${c.city}`
  );
});
