...
function RequestDetailPanel({ request, onClose, onSubmitCompletion }) {
  ...
  useEffect(() => {
    ...
    refurbishmentService
      .getPartnerRequestDetails(request.request_id)
      .then((res) => {
      ...
      <div className="flex items-start justify-between px-5 py-4 border-b border-gray-100 shrink-0">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400">
            Submitted {fmtDateTime(req?.created_at || request?.created_at)}
          </p>
        </div>
        <button
          onClick={onClose}
          className="ml-2 shrink-0 p-1.5 rounded-full hover:bg-gray-100 text-gray-400 hover:text-gray-700 transition-colors"
        >
          ...
          <div className="flex flex-col gap-3">
            ...
            {req?.has_admin_modifications && (
              <div className="mx-4 mt-4 px-4 py-2.5 rounded-xl bg-blue-50 border border-blue-200 text-xs text-blue-800">
                <span className="font-semibold">Admin Note:</span> The admin has
                curated the package list for this request.
              </div>
            )}
            {/* Admin remarks / rejection */}
            {(req?.admin_remarks || req?.rejection_reason) && (
              <div
                className={`mx-4 mt-4 px-4 py-3 rounded-xl border text-xs ${req?.status === "rejected" ? "bg-red-50 border-red-200 text-red-800" : "bg-amber-50 border-amber-200 text-amber-800"}`}
              >
            ...
            {showCompletion && (
              <div className="mx-4 mt-2 px-4 py-3 rounded-xl bg-purple-50 border border-purple-200">
                <p className="text-xs text-purple-800 font-semibold mb-2">
                  Refurbishment Complete â€” Confirmation Required
                </p>
                <button
                  onClick={() => onSubmitCompletion(request)}
                  className="px-4 py-1.5 text-xs bg-purple-600 text-white rounded-full font-semibold hover:bg-purple-700 transition-colors"
                >
                  Submit Completion
                </button>
              </div>
                  ...
                  >
                    {tab.isUpgradation ? (
                      <span className="flex items-center gap-1">
                        <Ruler className="w-3 h-3" /> Upgradation
                      </span>
                    ) : (
                      tab.label
                    )}
              ...
              <div className="flex min-h-[200px] gap-3 mx-4 mb-3">
                ...
                <div className="w-44 shrink-0 space-y-1.5 overflow-y-auto">
                  {activeCourse.packages.length === 0 ? (
                    <p className="text-xs text-gray-400 text-center py-6">
                      No packages for this course.
                    </p>
                  ) : (
                    activeCourse.packages
                      .filter((p) => !p.removed_by_admin)
                      .map((pkg) => (
                        <button
                          key={pkg.package_id}
                        ...
                        >
                          <div className="flex items-center justify-between gap-1">
                            ...
                            {selectedPkg?.package_id === pkg.package_id && (
                              <ChevronRight className="w-3 h-3 shrink-0 text-green-600" />
                            )}
                          </div>
                          {pkg.added_by_admin && (
                ...
                {selectedPkg ? (
                  <div className="flex-1 overflow-y-auto rounded-xl border border-gray-100 bg-gray-50/60 p-4 space-y-4">
                    ...
                    {(() => {
                      const imgs = safeParse(selectedPkg.images);
                          ...
                          <div className="flex flex-wrap gap-2">
                            {imgs.map((url, i) => (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="w-16 h-16 rounded-lg overflow-hidden border border-gray-200 hover:border-green-400 transition-colors"
                              >
                                <img
                                  src={url}
                                  alt={`ref-${i}`}
            ...
            {isUpgradationTab && details.upgradation && (
              <div className="mx-4 mb-4 mt-2 space-y-4">
                {details.upgradation.rooms?.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="text-xs font-semibold text-gray-600 flex items-center gap-1.5">
                      <Ruler className="w-3.5 h-3.5 text-green-600" /> Room
                      Dimensions
                    </h4>
                ...
                {details.upgradation.selected_packages?.length > 0 && (
                  <div>
                    <h4 className="text-xs font-semibold text-gray-600 flex items-center gap-1.5 mb-2">
                    ...
                    <div className="space-y-2">
                      {details.upgradation.selected_packages.map((pkg) => (
                        <div
                          key={pkg.package_id}
                          className="bg-white border border-purple-200 rounded-xl p-3"
                        >
            ...
            {(details?.refurbishment_document ||
              ...
              <div className="mx-4 mb-4">
                <h4 className="text-xs font-semibold text-gray-600 flex items-center gap-1.5 mb-2">
                  <FileText className="w-3.5 h-3.5 text-green-600" /> Attached
                  Documents
                </h4>
                <div className="space-y-2">
                  {details.refurbishment_document && (
              ...
              !details.upgradation_requested && (
                <div className="py-14 text-center">
                  ...
                  <p className="text-gray-400 text-xs">
                    No package selection data found.
                  </p>
                </div>
              )}
          </div>
...
export default function PartnerPastRequestsTab() {
  ...
  const handleSelectRequest = (req) =>
    setSelectedRequest((prev) =>
      prev?.request_id === req.request_id ? null : req,
    );
  ...
  return (
    <div className="flex gap-4 min-h-0 h-full">
      ...
      <div
        className={`flex flex-col gap-3 transition-all duration-300 ${selectedRequest ? "w-[48%]" : "w-full"}`}
      >
        {/* Search + filter */}
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by center or request IDâ€¦"
          ...
          >
            {["All", "Submitted", "Approved", "Rejected", "Completed"].map(
              (o) => (
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-xs text-gray-400">
                          <span className="capitalize">
                            {req.type?.replace(/_/g, " ") || "Refurbishment"}
                          </span>
                          <span>Â·</span>
                      ...
                      <ChevronRight
                        className={`w-4 h-4 shrink-0 mt-1 transition-colors ${isSelected ? "text-green-600" : "text-gray-300"}`}
                      />
                    </div>
                  </button>
                );
